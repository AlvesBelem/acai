...
const heroStats = [
  { value: "+420t", label: "resíduos reaproveitados" },
  { value: "R$ 1,8 mi", label: "pagos a parceiros" },
  { value: "230+", label: "lojistas ativos" },
] as const;

const heroHighlights: Array<{ icon: LucideIcon; label: string }> = [
  { icon: ShieldCheck, label: "Certificado ambiental automático" },
  { icon: TimerReset, label: "Agenda flexível sob demanda" },
  { icon: BadgeDollarSign, label: "Pagamento por quilo coletado" },
  { icon: Leaf, label: "Destinação circular comprovada" },
];
...
const aboutHighlights: Highlight[] = [
  ...
  },
  {
    ...
    description: "Equipe confirma pelo WhatsApp e monitora o caminhão na rota.",
  },
  {
    icon: Leaf,
    title: "ESG na prática",
    description: "Certificados digitais fortalecem marketing e prestação de contas.",
  },
];

const fleetHighlights: Highlight[] = [
  {
    icon: ShieldCheck,
    title: "Pesagem aferida",
    description: "Balanças calibradas semanalmente e registro fotográfico da coleta.",
  },
  {
    icon: TimerReset,
    title: "Rotas inteligentes",
    description: "Janelas de coleta otimizadas para não parar o seu atendimento.",
  },
];
...
const steps: Step[] = [
  {
    ...
    description:
      "Entre com Google, informe endereço e volume estimado. Em minutos abrimos sua agenda de coleta.",
  },
  {
    title: "Coleta monitorada",
    description:
      "Nosso caminhão truck bancado chega no horário combinado e registra tudo com fotos e assinatura digital.",
  },
  {
    title: "Pagamento imediato",
    description:
  ...
  },
  {
    ...
    description:
      "Você recebe laudos ambientais e relatórios para compartilhar com clientes, fornecedores e times ESG.",
  },
];
...
const testimonials: Testimonial[] = [
  {
    quote:
      "Antes eu pagava para descartar. Agora recebo um bônus mensal e mostro o impacto para os clientes.",
...
const faqs: Faq[] = [
  {
    ...
    answer:
      "Você escolhe janelas de dia e horário no painel. Confirmamos pelo WhatsApp e o caminhão registra check-in e check-out no local.",
  },
  {
  ...
  },
  {
    ...
    answer:
      "Basta separar em sacos ou bombonas até a coleta. Quanto mais secos, maior o peso aproveitado na pesagem.",
  },
  {
    question: "Quais cidades atendem?",
    answer:
      "Atendemos São Paulo, Rio de Janeiro, Belo Horizonte e Vitória. Novas regiões são comunicadas diretamente no painel.",
  },
];
...
export default function Landing() {
  ...
  const handleSignIn = () =>
    startTransition(() => signIn("google", { callbackUrl: "/auth/redirect" }));
        ...
        <div className="relative mx-auto flex max-w-[1200px] flex-col gap-12 px-6 pb-20 pt-24 md:flex-row md:items-stretch md:gap-20">
          <div className="space-y-6 md:w-1/2">
            ...
            <h1 className="text-4xl font-bold leading-tight md:text-5xl">
              Ganhe com os caroços de açaí e tenha toda a logística cuidada
            </h1>
            <p className="max-w-xl text-lg text-[#F4E5FF]">
              Coletamos no seu endereço, pagamos pelo material e enviamos comprovantes ambientais. Tudo acompanhado pelo painel Açaí Coleta.
            </p>
            ...
            <div className="grid max-w-lg grid-cols-3 gap-6 pt-8 text-left">
              {heroStats.map((stat) => (
                <div key={stat.label}>
                  <p className="text-3xl font-bold text-[#F6A609]">{stat.value}</p>
                  <p className="text-sm text-[#F4E5FF]">{stat.label}</p>
                </div>
              ))}
            </div>
          </div>
          <div className="md:w-[55%]">
            <div className="flex h-full flex-col overflow-hidden rounded-3xl border border-white/10 bg-white/5 px-10 py-8 shadow-2xl shadow-black/20 backdrop-blur">
              <div className="grid h-full gap-6 md:grid-cols-2">
                ...
                <div className="flex h-full flex-col justify-between space-y-4 text-sm text-[#F4E5FF]">
                  <div>
                    ...
                    <p className="mt-3">
                      Frota própria adaptada para recolher caroços com pesagem aferida. Cada coleta gera protocolo com fotos e assinatura digital.
                    </p>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    {heroHighlights.map(({ icon: Icon, label }) => (
                      <CardBullet key={label} icon={<Icon className="h-4 w-4 text-[#F6A609]" />}>
                        {label}
                      </CardBullet>
                    ))}
                  </div>
              ...
              <div className="mt-8 grid gap-4 md:grid-cols-3">
                ...
                <div className="relative h-24 overflow-hidden rounded-2xl md:col-span-2">
                  <Image
                    src="/caminhao/caminhao3.jpeg"
                    alt="Caminhão Açaí Coleta na doca de carregamento"
                    fill
                    className="object-cover"
                  />
                </div>
        ...
        <div className="mx-auto grid max-w-6xl gap-10 px-6 md:grid-cols-[1.2fr_1fr] md:items-center">
          ...
          <div className="space-y-5">
            ...
            <p className="text-base text-[#4B0D66]">
              Desde 2014 conectamos lojistas e indústria para reutilizar caroços de açaí. Assumimos transporte, documentação ambiental e entregamos receita previsível para quem antes pagava para descartar.
            </p>
            <ul className="space-y-3 text-sm text-[#4B0D66]">
              {aboutHighlights.map(({ icon: Icon, title, description }) => (
                <li key={title} className="flex gap-3">
                  <Icon className="mt-1 h-5 w-5 text-[#7B2CBF]" />
                  <span>
                    <strong className="block text-[#2B0141]">{title}</strong>
                    {description}
                  </span>
                </li>
              ))}
      ...
      <section className="bg-[#2B0141] py-20">
        <div className="mx-auto max-w-6xl px-6">
          ...
          <p className="mt-3 text-center text-sm text-[#DCC4F7]">
            Do agendamento ao pagamento, tudo acontece em quatro etapas simples dentro da plataforma.
          </p>
          ...
          <div className="mt-14 flex flex-col items-center gap-6">
            ...
            <div className="relative grid w-full max-w-3xl grid-cols-4 gap-4">
              {flowLabels.map((label, index) => (
                <div
                  key={label}
                  className="relative flex flex-col items-center text-center text-xs text-[#F4E5FF]"
                >
        ...
        <div className="mx-auto max-w-6xl px-6">
          <div className="flex flex-col gap-10 md:flex-row md:items-center">
            <div className="space-y-4 md:w-1/2">
              <h2 className="text-3xl font-bold text-[#2B0141]">
                Nossa frota pronta para recolher
              </h2>
              <p className="text-sm text-[#4B0D66]">
                Caminhões preparados para operar em centros urbanos e rotas regionais. Mantemos
                higienização constante das carrocerias e monitoramos cada saída via painel de operações.
              </p>
              <ul className="space-y-3 text-sm text-[#4B0D66]">
                {fleetHighlights.map(({ icon: Icon, title, description }) => (
